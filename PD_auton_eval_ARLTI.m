%% Parkinson's Disease (PD) Project - evaluate AR/LTI full state systems
% Autonomous only: does not include brain stimulation. 
% not testing neural network

%% load data file 
[fn,fp] = uigetfile('*sysARLTI*.mat');
load(fullfile(fp,fn), 'sysLTI', 'sysAR', 'sysARLTI', ...
    'dataTrain', 'dataTest', 'dataTrainTapped', 'dataTestTapped');
disp([fp,' --- ',fn]);
[~,fn] = fileparts(fn);
sysName = {'sysAR', 'sysLTI', 'sysARLTI'};
sysColr = {'b',     'r',      'm'};
sys = cellfun(@eval,sysName, 'UniformOutput',false);
fsNew = dataTrain.Properties.SampleRate;

[~,dataTestSel] = max(cellfun(@height, dataTest));
dataTest = dataTest{dataTestSel}; dataTestTapped = dataTestTapped{dataTestSel};
...
dataTestTapped = {dataTestTapped(1:Lval,:)}; dataTrainTapped = dataTrainTapped(1:Lval,:);
%}
...
for hzn = hzns
    ...
    %%{
    disp('Training Eval');
    ypTrain_ = cellfun(@(S) myPredict(S, dataTrain, hzn, true), sys(1:2), ...
        'UniformOutput',false);
    ypTrainARLTI = myPredict(sys{3}, dataTrainTapped, hzn, true, false);
    ypTrainARLTI = ypTrainARLTI(:,1:width(dataTrain));
    ypTrain_ = [ypTrain_, {ypTrainARLTI}];
    ypTrain = [ypTrain; ypTrain_];
    %}
    disp('Testing Eval');
    ypTest_ = cellfun(@(S) myPredictCell(S, dataTest, hzn, true), sys(1:2), ...
        'UniformOutput',false);
    ypTestARLTI = myPredictCell(sys{3}, dataTestTapped, hzn, true);
    ypTestARLTI = ypTestARLTI(:,1:width(dataTrain));
    ypTest_ = [ypTest_, {ypTestARLTI}];
    ypTest = [ypTest; ypTest_];
end
clear ypTrain_ ypTest_ ypTrainARLTI ypTestARLTI
...
for k = 1:K
    ...
    legend({'AR', 'LTI', 'AR-LTI'}, "Location","best");
...
dataTrainRT = retime(dataTrain, 'regular', 'nearest', 'TimeStep', seconds(sys{2}.Ts));
dataTrainTappedRT = retime(dataTrainTapped, 'regular', 'nearest', 'TimeStep', seconds(sys{3}.Ts));
dataTrainTappedRT.Time = dataTrainTappedRT.Time - dataTrainRT.Time(1);
dataTrainRT.Time = dataTrainRT.Time - dataTrainRT.Time(1);

ysLTI = sim(sys{2}, dataTrainRT(1:hzn, []), ...
...
for c = 1:H
    ...
    plottbl(ysARLTI, ch(c), sysColr{3}, 1.75);
    legend('true', 'AR', 'LTI', 'AR-LTI');
